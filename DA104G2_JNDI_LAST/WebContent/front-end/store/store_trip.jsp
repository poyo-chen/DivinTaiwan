			...
			<div id="content">
				...
				<div class="container-fluid" id="container-wrapper">
					<div
						...
						<span class="trip_manage">行程管理</span> <a
							href="<%=request.getContextPath()%>/front-end/store/store_add.jsp"><input
							class="btn btn_new" type="submit" value="新增行程"> </a>
					</div>
					...
					<nav>
						<div class="nav nav-tabs" id="nav-tab" role="tablist">
							<a class="nav-item nav-link active all_trip" id="nav-home-tab"
								...
								aria-controls="nav-home" aria-selected="true">所有行程</a> 
							<a
								class="nav-item nav-link listed_trip" id="nav-profile-tab"
								data-toggle="tab" href="#nav-profile" role="tab"
								aria-controls="nav-profile" aria-selected="false">已上架</a> 
							<a
								class="nav-item nav-link unlisted_trip" id="nav-contact-tab"
								data-toggle="tab" href="#nav-contact" role="tab"
								aria-controls="nav-contact" aria-selected="false">未上架</a>
						</div>
					</nav>
									...
									<tbody>
										...
										<c:forEach var="tourVO" items="${list}"
											...
											<tr>
												...
												<td>${applicationScope.tourReleased[tourVO.tour_status]}</td>
												...
												<td>
													<form method="post" action="<%= request.getContextPath()%>/tour/tour.do">
													<input type="hidden" name="tour_no" value="${tourVO.tour_no}">
													<input type="hidden" name="action" value="getTourUpdate">
							...
							<div>
								<table class="table text-center">
									...
									<tbody>
										<c:forEach var="tourVO" items="${list}"
											begin="<%=pageIndex %>">
											<c:if test="${tourVO.tour_status == 1}">
												<tr>
													<th scope="row">${tourVO.tour_name}</th>
													<td>${tourVO.tour_price}</td>
													<td>${tourVO.tour_max_num}</td>
