...
<title>潛進台灣Taiwan Divers 購物商城</title>
        ...
        <div class="container-fluid">
            <div class="row offset-1">
                <div class="col-6">
                    <div class="row">
                        <div id="big_img">
                            <img src="<%=request.getContextPath()%>/ShowPic_V1?equip_no=${equipmentVO.equip_no}&equip_img=1" style="width: 400px;">
                        </div>  
                    </div>
                    <div class="row offset-2">
                        <div class="col-3" id="small_img">
                            <img src="<%=request.getContextPath()%>/ShowPic_V1?equip_no=${equipmentVO.equip_no}&equip_img=1" style="width: 100px" onclick="showBig()" id="1">
                        </div>
                        <div class="col-3" id="small_img">
                            <img src="<%=request.getContextPath()%>/ShowPic_V1?equip_no=${equipmentVO.equip_no}&equip_img=2" style="width: 100px" onclick="showBig()" id="2">
                        </div>
                        <div class="col-3" id="small_img">
                            <img src="<%=request.getContextPath()%>/ShowPic_V1?equip_no=${equipmentVO.equip_no}&equip_img=3" style="width: 100px" onclick="showBig()" id="3">
                        </div>
                    </div>
                </div>
                <div class="col-5" id="right_des" style="line-height: 46px;">
	                    <div style="margin-top: 10px">裝備名稱：${equipmentVO.equip_name}</div>
	                    <div style="margin-top: 10px">裝備詳情：${equipmentVO.equip_des}</div>
	                    <div style="margin-top: 10px">價格：<font color="red">NT$ ${equipmentVO.equip_price}</font></div>
	                    <div style="margin-top: 10px">付款方式：信用卡</div>    
	                    <div style="margin-top: 10px">更新日期：${equipmentVO.equip_update}</div>
	                    <div style="margin-top: 10px">數量： <input type="number" value="1" min="1" max="100" step="1" name="buy_amt" id="buy_amt" class="form-control"/></div>
	                    <div class="container">
	                        <div class="row" style="margin-top: 100px">
<!-- 	                            <div class="col-8"></div> -->
	                            <div class="col-12"><button type="submit" class="btn btn-success" id="addBtn">加入購物車</button></div>
	                        </div>
	                    </div>
                </div>
            </div>
	...
	$('#addBtn').click(function(){
		...
		if ($('#buy_amt').val() <= 0) {
			...
			$.ajax({
		        ...
		        data:{
			         action:"add",
			         equip_no:"${equipmentVO.equip_no}",
			         equip_name:"${equipmentVO.equip_name}",
			         buy_amt:$('#buy_amt').val(),
			         equip_price:"${equipmentVO.equip_price}",
		        },
...
<script type="text/javascript">
	...
	function showBig(){
		var picinfo=event.target.id;
 		var bigPic = document.getElementById("big_img").innerHTML="<img src=<%=request.getContextPath()%>/ShowPic_V1?equip_no=${equipmentVO.equip_no}&equip_img=" + picinfo + " style='width: 400px;' >"; 
