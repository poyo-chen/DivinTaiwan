...
<%
	...
	List<GroupVO> list = (List<GroupVO>) session.getAttribute("groupVO");
...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/css/listAllGroup.css">
<title>單一潛點揪團瀏覽</title>
<style>
#diverpoints {
...
<body>
						...
						<c:otherwise>
							<a class="user_name"
								href="<%=request.getContextPath()%>/front-end/memberCenter/memDetail.jsp"><span>
									...
									style="border-radius: 50%;" width='40px' height='40px'>
							</span> <span>${memVO.mem_name}</span> </a>
						</c:otherwise>
					</c:choose>
					<a
				...
				<c:when
					...
					<a class="user_name"
						href="<%=request.getContextPath()%>/front-end/store/store.jsp"><span><img
							...
							style="border-radius: 50%;" width='40px' height='40px'></span> <span>${memVO.mem_name}</span>
					</a>
					<a
						href="<%=request.getContextPath()%>/member/MemberServlet.do?action=logout"><button
							type="button" class="btn" id="btn_out" data-toggle="modal"
	...
	<!-- 熱門潛點導覽列 -->
	<div class="container" id="diverpoints">
		<div class="row" style="margin: 0px auto 0px auto;">
			<c:forEach var="diveVO" items="<%=DiveSvc.getAll()%>" begin="0"
				end="5">
				<div class="chose">
						...
						<form action="<%=request.getContextPath()%>/group/group.do">
							<button style="border: 0; background-color: #fff">
								<div class="card">
									<div class="card_img">
										<img
											src="<%=request.getContextPath()%>/ShowPic_V1?group_no=${groupVO.group_no}">
									</div>
									<div class="card-body">
									...
									<div class="card-footer">
										<small class="text-muted">倒數
											${groupVO.group_tour_stop_time}</small> <small
											class="text-muted people">報名人數${groupVO.group_tour_num}
											/ ${groupVO.group_max_num}</small>
									</div>
								</div>
