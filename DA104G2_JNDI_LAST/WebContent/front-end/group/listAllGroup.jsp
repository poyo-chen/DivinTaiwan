...
<%
	...
	List<GroupVO> list = (List<GroupVO>) request.getAttribute("groupVO");
	if (list==null) { 
		...
		for (int i = 0; i < list.size(); i++) {
			GroupVO g = list.get(i);
			GroupVO groupVO = groupSvc.getOneGroup(g.getGroup_no());
...
%>
<jsp:useBean id="DiveSvc" scope="page" class="com.dive.model.DiveService" />
<html lang="zh-Hant">
<head>
<meta charset=" utf-8">
...
<body>
			...
			<div class="fast">
				<h2 class="section-title">
					<img class="title-icon-box"
						src="<%=request.getContextPath()%>/images/people.png" alt="">熱門潛點
				</h2>
				<hr>
			</div>
		</div>
	...
	<div class="container">
		<div class="row" style="margin: 0px auto 0px auto;">
			<c:forEach var="diveVO" items="<%=DiveSvc.getAll()%>" begin="0" end="5"> 
				<div class="chose">
					<form action="<%=request.getContextPath()%>/group/group.do">
						<button style="border: 0; background-color: #fff">
							<figure>
								<img src="<%=request.getContextPath()%>/ShowPic_V1?dive_no=${diveVO.dive_no}">
							</figure>
							<h6>${diveVO.dive_name}</h6>
						</button>
						<input type="hidden" name="dive_no" value="${diveVO.dive_no}">
						 <input type="hidden" name="action" value="getOne_For_Dive">
					</form>
				</div>
			</c:forEach>	
		</div>
		...
		<div class="row">
			...
			<div class="btn-group btn-group-toggle col-4" data-toggle="buttons">
				<form action="<%=request.getContextPath()%>/group/group.do">
					<button class="btn btn-outline-primary">北部</button>
					<input type="hidden" name="WeatherAreaNo" value="WA00004"> 
					<input type="hidden" name="action" value="getArea">
				</form>
				<form action="<%=request.getContextPath()%>/group/group.do"> 
					<button class="btn btn-outline-secondary" name="action" value="getArea">東部</button>
					<input type="hidden" name="WeatherAreaNo" value="WA00007"> 
					<input type="hidden" name="action" value="getArea">
				</form>
				<form action="<%=request.getContextPath()%>/group/group.do">
					<button class="btn btn-outline-success">南部</button>
					<input type="hidden" name="WeatherAreaNo" value="WA00001"> 
					<input type="hidden" name="action" value="getArea">
				</form>
				<form action="<%=request.getContextPath()%>/group/group.do">
					<button class="btn btn-outline-danger">澎湖</button>
					<input type="hidden" name="WeatherAreaNo" value="WA00006"> 
					<input type="hidden" name="action" value="getArea">
				</form>
			</div>
			<!-- 搜尋關鍵字 -->
			<div class="input-group mb-3 col-8">
				<form id="searchform" action="<%=request.getContextPath()%>/group/group.do">
					<input type="text" id="search" name="word" placeholder="　揪團名稱"
						aria-label="Recipient's username" aria-describedby="button-addon2">
					<div class="input-group-append">
						<button name="action" class="btn btn-outline-secondary"
							type="submit" id="button-addon2" value="likesearch">
		...
		<!-- 揪團列表 -->
		<div class="total_group"><%@ include file="groupAllPage1.file"%></div>
		<div class="card-deck">
			...
			<c:forEach var="groupVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
				...
				<form action="<%=request.getContextPath()%>/group/group.do">
					<button style="border: 0; background-color: #fff">
						<div class="card">
							<div class="card_img">
								<img src="<%=request.getContextPath()%>/ShowPic_V1?group_no=${groupVO.group_no}">
							</div>
							<div class="card-body">
								<h5 class="card-title">${groupVO.group_name}</h5>
								<p class="card-text">${groupVO.group_des}</p>
							</div>
							<div class="card-footer">
								<small class="text-muted people">報名人數${groupVO.group_tour_num} / ${groupVO.group_max_num}</small> <small class="text-muted" id="look">
									<input value="查看詳情" type="button"></small>
							</div>
						</div>
					</button>
	...
	<a href="<%=request.getContextPath()%>/front-end/group/addGroup.jsp">
		<div id="service">
			<div class="add_group">創建揪團</div>
			<i class="fas fa-swimmer"></i>
		</div>
	</a>
	
	<!-- 新增潛點++ -->
	<a href="<%=request.getContextPath()%>/front-end/dive/addDiveMap.jsp">
		<div id="addDive">
			<div class="add_group">新增潛點</div>
			<i class="fas fa-anchor"></i>
		</div>
	</a>
	
	<!-- 個人揪團++ -->
	<a href="<%=request.getContextPath()%>/group/group.do?action=mygroup&mem_no=${memVO.mem_no}">
		<div id="myGroup">
			<div class="add_group">個人揪團</div>
